<?php
	...
	/* why did i do this?
	$qq = mysqli_query("SELECT COLUMN_NAME(PASSWORD) FROM TABLE_NAME WHERE USERID = 'SOMEUSERID'");
	$confirm = mysqli_fetch_assoc($qq);
...
?>
	...
	<div id="sideNav">
		...
		<form id="newFolderForm" action="actions/newFolder.php" method="post">
			<input id="newFolder" name="newFolder" type="text" placeholder="New Folder" style="margin-left:10px;" required="required" />
		...
		<!-- <tr>
			<td id="userFolders"> -->
				<?php // start PHP
					...
					// create query to populate ('#userFolders').
					$folderTitles = "SELECT * FROM folders WHERE hidden = 0 ORDER BY siteName asc";
					$results = mysqli_query($conn, $folderTitles);

					while ($row = mysqli_fetch_assoc($results)) {
						foreach ($row as $field)  {
							echo('<tr class="folders" onclick="somethings()">');
								echo('<td class="userFolders">');
									echo('<span class="userFolderIcon"><i class="glyphicon glyphicon-folder-close"></i>');
										echo(htmlspecialchars(' ' . $field['siteName']));
									echo('</span>');

									echo('<form action="actions/renameFolder.php" method="POST"><button class="btn renameFolder" title="Rename Folder"><i class="glyphicon glyphicon-pencil"></i></button></form>');
									echo('<form action="actions/deleteFolder.php" method="POST"><button class="btn deleteFolder" title="Delete Folder"><i class="glyphicon glyphicon-trash"></i></button></form>');
